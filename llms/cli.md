...
The `pspt` command-line tool: compiles/runs `.pspt` DSL files (see
[dsl.md](./dsl.md)) and scans git history for use in trackers.
...
`pspt-docx`/`pspt-xlsx` setters — see [dsl.md](./dsl.md) for what the generated
code looks like). Does **not** run it.

Prints `Compiled <file> -> <file>.gen.js (type=<docType>)` on success, where
`<docType>` is the `type=` declared on the file's `doc` line — `master`, one of
the fourteen Part keys (e.g. `projectBrief`), or `xlsx`.
...
The `--out` directory must already exist — `pspt` does not create it for you.

If `--out` is omitted, the default is derived from the declared `type=`:
`product-documentation-master.docx` for `type=master`, `<partKey>.docx` for a
single Part, `output.xlsx` for `type=xlsx`.
...
| `--with-lines`   | off             | Also runs `git show --shortstat` **per commit** to attach `{insertions, deletions}` to each entry. Costs one extra git process per commit — fine for tens/hundreds of commits, slow for very large histories. Omit it if you only need commit metadata (hash/date/author/subject). |
...
values into your `.pspt` file's `task` attributes (see [dsl.md](./dsl.md)), or
compute a summed total across several commit hashes for one task with
`pspt-core`'s `getCommitStats` (see [core.md](./core.md)):
...
See [xlsx-sdk.md](./xlsx-sdk.md#task-fields) for where `commitCount`/`lines`
land in the generated spreadsheet.

## Usage / help

Running `pspt` with no command, or an unrecognized one, prints full usage and
exits non-zero (zero if no command was given at all).
...
writing the output path, such as a non-existent `--out` directory).
