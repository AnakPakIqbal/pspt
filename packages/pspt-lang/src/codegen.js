...
function itemsToJs(items, indent) {
  ...
  const lines = items.map((item) => {
    if (item.children && item.children.length) {
      const childLines = item.children.map((child) => `${childPad}${jsLiteral(child.title)}`);
      return `${pad}{ title: ${jsLiteral(item.title)}, children: [\n${childLines.join(',\n')}\n${pad}] }`;
    }
    return `${pad}{ title: ${jsLiteral(item.title)} }`;
  });
  return `[\n${lines.join(',\n')}\n${'  '.repeat(indent - 1)}]`;
}
...
function generateDocx(program, sourceFileLabel) {
  ...
  function rowObjLiteral(row, indent) {
    ...
    const fields = row.fields.map((fld) => `${fieldPad}${fld.key}: ${valueNodeToJs(fld.value)}`);
    return `{\n${fields.join(',\n')}\n${pad}}`;
  }
  ...
  function rowsArrayLiteral(rowsNode, indent) {
    ...
    const indentedItems = items.map((i) => `${pad}  ${i}`);
    return `[\n${indentedItems.join(',\n')}\n${pad}]`;
  }
  ...
  program.statements.forEach((stmt) => {
    ...
    plainFields.forEach((fld) => {
      if (fld.name === 'notes' && hasMeasuresList) return;
      const method = SETTER_DATA_KEY_TO_METHOD[fld.name];
      if (!method) {
        lines.push(
          `  // WARNING: unrecognized field '${fld.name}' (line ${fld.line}) — no matching setter, skipped`,
        );
        return;
      }
      if (OBJECT_PAYLOAD_KEYS.has(fld.name) && fld.value.kind !== 'null') {
        ...
        lines.push(
          `  // WARNING: field '${fld.name}' (line ${fld.line}) maps to ${method}, which expects an object (e.g. {productName, ...}), not a plain value — skipped. Nested object fields are not yet supported by this grammar.`,
        );
        return;
      }
      lines.push(`  doc.${method}(${valueNodeToJs(fld.value)});`);
    });
    ...
    tablesByName.forEach((tableNode, name) => {
      ...
      if (!method) {
        lines.push(
          `  // WARNING: table '${name}' (line ${tableNode.line}) has no matching setter, skipped`,
        );
        return;
      }
    ...
    stmt.body.forEach((node) => {
      if (node.type === 'Rows' && !consumedRowNames.has(node.name)) {
        lines.push(
          `  // WARNING: rows '${node.name}' (line ${node.line}) has no matching 'table ${node.name} { ... }' in this section, skipped`,
        );
      }
    });
    ...
    Object.entries(objectFields).forEach(([listName, listNode]) => {
      if (listName === 'measures') {
        const notesField = plainFields.find((fld) => fld.name === 'notes');
        ...
        lines.push(
          `  // WARNING: list '${listName}' (line ${listNode.line}) has no known consumer, skipped`,
        );
      }
    });
